...
void main() {
  ...
  group('EncryptionService E2EE Phase 1', () {
    ...
    test(
      'generateNewKeyPair creates real RSA keys and exports PEM properly',
      () async {
    ...
    test(
      'encryptMessageForMultipleDevices returns AES-256-GCM ciphertext and RSA-OAEP envelopes',
      () async {
        await encryptionService.initializeKeys();
        ...
        final pubB = RsaKeyHelper().encodePublicKeyToPemPKCS1(
        ...
        final devices = [
          {
            'device_id': encryptionService.deviceId,
            'public_key': encryptionService.publicKeyPem,
          },
          {'device_id': deviceIdB, 'public_key': pubB},
        ];

        final result = encryptionService.encryptMessageForMultipleDevices(
          'Secret Message',
          devices,
        );
    ...
    test(
      'decryptMessageMultiDevice fails safely on corrupted ciphertext',
      () async {
        await encryptionService.initializeKeys();
        final devices = [
          {
    ...
    test(
      'decryptMessageMultiDevice fails safely on corrupted encrypted_key (RSA-OAEP validation)',
      () async {
        await encryptionService.initializeKeys();
        final devices = [
          {
