...
void main() {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  test('401 retry logic: succeeds when refresh returns a new access token', () async {
      ...
      if (options.path.contains('/protected')) {
        ...
        if (auth == 'Bearer old_access_token') {
          return ResponseBody.fromString(
            '{"status":false,"message":"unauthorized"}', 401,
            headers: {Headers.contentTypeHeader: ['application/json']},
          );
        }
        if (auth == 'Bearer new_access_token') {
          return ResponseBody.fromString(
            '{"status":true,"data":"success"}', 200,
            headers: {Headers.contentTypeHeader: ['application/json']},
          );
        }
      }
    ...
    final refreshAdapter = _MockAdapter((options) async {
      if (options.path.contains('/auth/refresh')) {
        refreshCalls++;
        return ResponseBody.fromString(
          '{"status":true,"data":{"access_token":"new_access_token"}}', 200,
          headers: {Headers.contentTypeHeader: ['application/json']},
        );
      }
      return ResponseBody.fromString('{}', 404);
    ...
    expect(protectedCalls, 2, reason: 'initial 401 + successful retry = 2 calls');
    expect(await storage.getToken(), 'new_access_token',
  ...
  test('401 retry limit: request is not retried more than once', () async {
    ...
    final refreshAdapter = _MockAdapter((options) async {
      if (options.path.contains('/auth/refresh')) {
        refreshCalls++;
        return ResponseBody.fromString(
          '{"status":true,"data":{"access_token":"refreshed_token"}}', 200,
          headers: {Headers.contentTypeHeader: ['application/json']},
        );
      }
      return ResponseBody.fromString('{}', 404);
  ...
  test('refresh failure triggers storage cleanup and signs user out', () async {
    ...
    expect(authSignedOutNotifier.value, true,
