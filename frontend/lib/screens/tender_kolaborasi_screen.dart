            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Padding(
                  ...
                  child: FilterChip(
                    label: Text(f, style: TextStyle(fontSize: 12, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, color: isSelected ? AppTheme.primaryPurple : null)),
                    selected: isSelected,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: Text(t['title'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14))),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                decoration: BoxDecoration(color: t['status_color'].withValues(alpha: 0.12), borderRadius: BorderRadius.circular(20)),
                child: Text(t['status'], style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: t['status_color'])),
              ),
            ],
          ),
          const SizedBox(height: 8),
          Text(t['vendor'], style: TextStyle(fontSize: 12, color: isDark ? AppTheme.textMuted : Colors.grey.shade600)),
          const SizedBox(height: 4),
          Row(
            children: [
              Icon(Icons.category_outlined, size: 12, color: isDark ? AppTheme.textMuted : Colors.grey.shade500),
              const SizedBox(width: 4),
              Text(t['category'], style: TextStyle(fontSize: 11, color: isDark ? AppTheme.textMuted : Colors.grey.shade500)),
              const Spacer(),
              Icon(Icons.calendar_today_outlined, size: 12, color: isDark ? AppTheme.textMuted : Colors.grey.shade500),
              const SizedBox(width: 4),
              Text(t['deadline'], style: TextStyle(fontSize: 11, color: isDark ? AppTheme.textMuted : Colors.grey.shade500)),
            ],
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              Icon(Icons.monetization_on_outlined, size: 14, color: AppTheme.primaryPurple),
              const SizedBox(width: 4),
              Text(t['budget'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
              const Spacer(),
              OutlinedButton(
                onPressed: () {},
                style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 6), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
